...
function scene:create( event )
	...
	composer.setVariable("specialSuccess3", specialSuccess3)
	-- 성공 소요 시간 합산
	local thirdDay = 0
	------------------------------------
	-- 문
	...
	local timeAttack
 	------------------------------------
	--제조성공시 점수
	local score_s = display.newText("성공 : " .. 0 .. " /  15", 660, 660) --성공시 점수
	score_s.x = 410
	score_s.y = 520
	score_s.size = 80
...
----------------------주문 랜덤 생성 ------------------------
	...
	local function orderMenu() 				
		...
		elseif success < 15 and fail < 5 then									-- 성공 횟수와 실패 횟수가 각각 20번, 5번 미만이라면 타이머를 시작합니다. (게임 종료 시 타이머 기능을 사용하지 않기 위해서 조건문 사용.)
...
------------------손님 이동----------------------------------
	...
	function newCustomerEnter() 
		...
		-- 특별한 손님 유뮤
		if success == 12 and specialCustomer == 0 then
			specialCustomer = 1
	 			...
	 			if  987 < event.target.x and event.target.x < 1320
					...
					--audio.play(coffee_down, {channel=1})
 					...
 					if pickMenu == drinkMade and count ~= 10 then
 						-- 소요 시간 더하기
 						thirdDay = thirdDay + count
 						print("소요 시간: " .. thirdDay)
 						success = success + 1
 						--- 20번 성공 시 이동
						score_s.text = "성공 : " .. success .. " /  15"
						if (success == 15) then
							-- 소요 시간 전달
							composer.setVariable( "thirdDay", thirdDay )
					...
					----------- 특별한 손님 특별한 메뉴 성공
					elseif specialCnt == 1 and drinkMade == 7 and count ~= 10 then
						--특별한 손님 특별한 메뉴 제공 성공
						specialSuccess3 = 1
						composer.setVariable("specialSuccess3", specialSuccess3)
 						-- 소요 시간 더하기
