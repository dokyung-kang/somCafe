...
function scene:create( event )
	...
	local timeAttack
 	------------------------------------
	--제조성공시 점수
	local score_s = display.newText("성공 : " .. 0 .. " /  7", 660, 660) --성공시 점수
	score_s.x = 410
	score_s.y = 520
	score_s.size = 80
	...
	tableGroup.alpha = 0
	local recipe_popup 
	recipe_popup = display.newImage(tableGroup, "img/recipe_background1.png", display.contentCenterX, display.contentCenterY)
...
----------------------주문 랜덤 생성 ------------------------
	...
	local function orderMenu() 				
		...
		elseif success < 7 and fail < 5 then									-- 성공 횟수와 실패 횟수가 각각 20번, 5번 미만이라면 타이머를 시작합니다. (게임 종료 시 타이머 기능을 사용하지 않기 위해서 조건문 사용.)
...
------------------손님 이동----------------------------------
	...
	function newCustomerEnter() 
		...
		-- 특별한 손님 유뮤
		if success == 4 and specialCustomer == 0 then
			specialCustomer = 1
